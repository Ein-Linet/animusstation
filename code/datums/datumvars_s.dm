...
client
	proc/debug_variables_s(datum/D in world)
		...
		if(istype(D, /atom))
			...
			#ifdef VARSICON
			if (A.icon)
				body += debug_variable_s("icon", new/icon(A.icon, A.icon_state, A.dir), 0)
			#endif
		...
		for (var/V in names)
			body += debug_variable_s(V, D.vars[V], 0, D)
	...
	proc/debug_variable_s(name, value, level, var/datum/DA = null)
		var/html = ""
...
*/
	...
	//
			...
			if (L.len > 0 && !(name == "underlays" || name == "overlays" || name == "vars" || L.len > 500))
				...
				if (0) // (L.vars.len > 0)
					html += "<ol>"
					for (var/entry in L)
						html += debug_variable_s(entry, L[entry], level + 1)
					html += "</ol>"
				else
					html += "<ul>"
					for (var/index = 1, index <= L.len, index++)
						html += debug_variable_s("[index]", L[index], level + 1)
					html += "</ul>"
		else
			html += "[name] = <span class='value'>[value]</span>"
	...
	Topic(href, href_list, hsrc)
		...
		if (href_list["Vars"])
			debug_variables_s(locate(href_list["Vars"]))
			...
			if(!istype(DAT,/datum) && !istype(DAT,/client))
				...
				return
			modify_variables(DAT, href_list["varnameedit"], 1, 1)
			...
			if(!istype(DAT,/datum) && !istype(DAT,/client))
				usr << "Can't edit an item of this type. Type must be /datum or /client, so anything except simple variables."
				return
			modify_variables(DAT, href_list["varnamechange"], 0, 1)
			...
			if(!istype(A,/atom))
				...
				return
			cmd_mass_modify_object_variables(A, href_list["varnamemass"], 1)
